...
eyJhbGciOiJSUzI1NiIsImtpXXXXXXXXXXXXXX
----
====
+
Copy the generated token to the clipboard for use in the next steps.

. Edit the `rhdh-secrets` Secret resource in the `devhub` namespace (You should have created this secret in previous labs)

.. Select "Secret" in Red Hat OpenShift Console:
+
image::secret.png[title=Select Secret]

.. Select *rhdh-secrets*, select *YAML* tab and add a new key named `K8S_SA_TOKEN` and set it's value to the token you generated in the previous step.
+
image::yaml_secret.png[title=Set K8S_SA_TOKEN secret]
+
You will refer to this secret as an environment variable in the `app-config.yaml` file.
...
Example:
+
image::kuber.png[title=Kubernetes Settings]
...
Example:
+
image::plugin_kuber.png[title=Enable Kubernetes Plugin]
...
.. Select the `Create > Register Existing Component` page from Red Hat Developer Hub.
+
image::template.png[title=Import Component]

.. Provide the full path to the https://github.com/RedHatQuickCourses/devhub-qc-apps/blob/main/kube/6-catalog-info.yml[6-catalog-info.yml] file from the previous step and click *Analyse*:
+
image::analyse.png[title=Analyse URL]

.. Click *Import*:
+
image::import.png[title=Import Component]

.. After the component is imported, verify that the component is listed in the `Catalog` page and click on `Nginx example service` to view the component details. You can safely ignore any warning messages about relations to other entities.
+
image::view.png[title=View Component]
...
* https://developers.redhat.com/rhdh/plugins#redhatnotpreinstalled[Plugins available in RHDH 1.4^]