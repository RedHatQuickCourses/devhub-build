...
As you already learned in the first introductory course in the learning path, RHDH as a platform consists of a small modular core and an extensive set of plugins that provide most of the functionality. In the upstream *Backstage.io* platform, enabling a plugin means modifying the platform's source code to load a bunch of TypeScript/JavaScript, and other static files (HTML, CSS, images, etc.). You then have to stop and restart the runtime to enable the plugin.
