...
. *Entity Providers* are responsible for communicating with different types of entities and importing raw data into RHDH.
...
<7> The scheduled interval at which RHDH should fetch and update catalog information. (Optional)
...
The information contained in the catalog YAML files is presented on the `Catalog` page of RHDH. You can filter and search for component information, and use various built-in and custom plugins work on processing the items in the Catalog.
...
. Restart the rollout of the RHDH pod and wait for the RHDH pods to be recreated. RHDH will auto-discover your catalog YAML descriptor files under your GitHub Organization based on the `schedule` configuration. Inspect the RHDH container logs if you see any errors fix the configuration in `app-config-rhdh` and redeploy. A successful scan by the plugin should result in logs like the following:
+
```
