...
. Provide `https://github.com/RedHatQuickCourses/devhub-qc-apps/blob/main/templates/react-ssr/template.yaml` as the value in the `Select URL` field.
+
image::url2.png[title=Select URL]

. Click `Analyze`. If the YAML is valid, you will be shown the results of the analysis.
+
image::url2review.png[title=Analyse results]
+
Otherwise, you will get a descriptive error message. Fix the error in the YAML files, and proceed with the import.
...
+
image::template.png[title=Imported Template]
...
+
NOTE: If you see don't see the "Choose" button then make sure your RBAC role contains the plugin `scaffold` with permission `scaffolder.task.create`.
...
+
image::fields.png[title=Template Input - Simple Information]
+
NOTE: If you see don't see the fields like *Name*, *Description* etc then make sure your RBAC role contains the plugin `scaffold` with permission `scaffolder.template`.

. For the `Choose a location` step, the `Host` field is disabled and the default value from the template is filled in. Enter the name of your GitHub Organization in the `Owner` field, and `test-react-app` in the `Repository` field (this value identifies the name of the GitHub repository). Click `Review` when done.
+
image::review.png[title=Repository Details]
+
